...
/* ROM specific bits: */
    ...
    {   "shield",       shield_flags,    "Mobile Shields."               }, /* Therin 99 */
    ...
    if ((value = flag_value (exit_flags, argument)) != NO_FLAG)
    {
        ...
        if (value == EX_ISDOOR) {
          if ( IS_SET(pRoom->exit[door]->exit_info, EX_ISDOOR) ) {
	          if ( pRoom->exit[door]->keyword && pRoom->exit[door]->keyword[0] == '\0' ) pRoom->exit[door]->keyword = "door";
          } else {
            if (pRoom->exit[door]->keyword == "door") pRoom->exit[door]->keyword[0] = '\0';
          }
        }
...
OEDIT( oedit_show )
{
    ...
    sprintf( buf, "eLevel:      [%5d]\n\r", pObj->effective_level );
    send_to_char( buf, ch );
...
OEDIT( oedit_addapply )
{
	...
	*/
    ...
    {
	send_to_char( "Syntax:  addapply [type] [#xmod] [bitvector]\n\r", ch );
	return FALSE;
    }
...
OEDIT( oedit_level )
{
    ...
    if ( pObj->effective_level < 1)
       pObj->effective_level= pObj->level;
    
    printf_to_char(ch, "Level set to %d.\n\r", pObj->level);
    return TRUE;
}
...
OEDIT( oedit_elevel )
{
    ...
    if ( argument[0] == '\0' || !is_number( argument ) )
    {
	send_to_char( "Syntax:  elevel [number]\n\r", ch );
	return FALSE;
    }

    pObj->effective_level = atoi( argument );

    printf_to_char(ch, "Effective level set to %d.\n\r", pObj->effective_level);
    return TRUE;
}
...
MEDIT( medit_show )
{
    ...
    sprintf( buf, "ELevel:      [%2d]\n\r",
	pMob->effective_level );
    send_to_char( buf, ch );
...
MEDIT( medit_level )
{
    ...
    if ( pMob->effective_level < 1)
      pMob->effective_level = pMob->level;
...
MEDIT( medit_elevel )
{
    ...
    pMob->effective_level = atoi( argument );

    printf_to_char(ch, "Effective level set to %d.\n\r", pMob->effective_level);
    return TRUE;
}
...
MEDIT( medit_shield )
{
	...
	if ( argument[0] != '\0' )
	{
		...
		if ( ( value = flag_value( shield_flags, argument ) ) != NO_FLAG )
		{
			...
			send_to_char( "Shield toggled.\n\r", ch );
			return TRUE;
		}
    }
    send_to_char( "Syntax: shield [flags]\n\r"
...
OEDIT( oedit_autoweapon )
{
  ...
  level = pObj->effective_level;
	...
	if (level < 1)
	{
		send_to_char( " {cAutoweapon requires an elevel to be set on the weapon first.{x\n\r", ch);
		return FALSE;
	}
   bonus = UMAX(0, level/10 - 1);

   // adjust this to change the avg dmg
	avg = (level * 0.8);
	dice = (level/10 + 1);
	size = dice/2;
  ...
  if (IS_SET(pObj->value[4],WEAPON_TWO_HANDS)) {
	...
	//pObj->cost = 25 * (size * (dice + 1)) + 20 * bonus + 20 * level;
  
	pObj->cost = level/3;
  pObj->cost *= level * 0.04;

  if (pObj->cost < 5)
    pObj->cost = 5;

	pObj->weight = level/2;
  pObj->weight *= level * 0.0025;
...
MEDIT( medit_autoset )
{
  ...
  level = pMob->effective_level;
	if (level < 1)
	{
		send_to_char("Set an elevel on the mob first!!!\n\r", ch);
		return FALSE;
	}

	// def
	ac_n = 95 - (level * 6.67) - ((level/10)^2);
	ac_x = 95 - (level * 4.57) - ((level/10)^2);
	...
	if (dice < 1)
		dice = 1;
	size = (level/12) * (0.0015 * level);
	if (size < 2)
		size = 2;
	bonus = 3 + ((level/2) * (0.01 * level));
	sprintf(temp, "%dd%d + %d", dice, size, bonus);
	medit_damdice(ch, temp);

	// hp
	bonus = (level/50 + 1) * ((level * 4) + (level/10)) +1; 
	size = level/3;
	if (size < 2)
		size = 2;
	dice = level + (level * 1.5);
	if (dice < 1)
		dice = 1;
	sprintf(temp, "%dD%d + %d", dice, size, bonus);
...
MEDIT( medit_autohard )
{
  ...
  level = pMob->effective_level;
	...
	// def
	ac_n = 88 - (level * 7.05) - ((level/10)^2);
	ac_x = 88 - (level * 5.02) - ((level/10)^2);
	pMob->ac[AC_PIERCE] = ac_n;
	pMob->ac[AC_BASH]   = ac_n;
	pMob->ac[AC_SLASH]  = ac_n;
	pMob->ac[AC_EXOTIC] = ac_x;
	...
	if (dice < 1)
		dice = 1;
	dice++;
	size = (level/12) * (0.002 * level);
	if (size < 2)
		size = 2;
	bonus = 10 + ((level/2) * (0.02 * level));
	sprintf(temp, "%dd%d + %d", dice, size, bonus);
	medit_damdice(ch, temp);

	// hp
	bonus = (level/50 + 1) * ((level * 12) + (level/10)) +1; 
	size = level/3;
	if (size < 2)
		size = 2;
	size++;
...
MEDIT( medit_autoeasy )
{
  ...
  level = pMob->effective_level;
	...
	ac_n = 99 - (level * 6.37) - ((level/10)^2);
	ac_x = 99 - (level * 4.27) - ((level/10)^2);
	...
	if (dice < 1)
		dice = 1;
	size = (level/18) * (0.002 * level);
	if (size < 2)
		size = 2;
	bonus = 1 + ((level/2) * (0.005 * level));
	sprintf(temp, "%dd%d + %d", dice, size, bonus);
	medit_damdice(ch, temp);

	// health
	bonus = (level/50 + 1) * ((level * 2) + (level/11)) +1; 
	size = level/3;
	if (size < 2)
		size = 2;
	dice = level + (level * 1.15);
	if (dice < 1)
		dice = 1;
	sprintf(temp, "%dD%d + %d", dice, size, bonus);
	medit_hitdice(ch, temp);
 ...
 HEDIT (hedit_new)
 {
     ...
     if (argument[0] == '\0')
     {
         send_to_char("Syntax: hedit new [keyword(s)]\n\r",ch);
         return FALSE;
     }
