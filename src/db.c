	   ...
	   else if ( letter == 'C') /* clan */
	   {
		...
		case 4: pexit->exit_info = EX_ISDOOR|EX_NOPASS|EX_PICKPROOF;
			pexit->rs_flags  = EX_ISDOOR|EX_NOPASS|EX_PICKPROOF;
                    case 5:
                        pexit->exit_info =  EX_ISDOOR|EX_CLOSED|EX_HIDDEN;
                        pexit->rs_flags =  EX_ISDOOR|EX_CLOSED|EX_HIDDEN;
                        break;
			break;
		}
...
/*
		    ...
		    &&   pObj->level < LastMob->level -5 && pObj->level < 45))
              &&  (obj->pIndexData->vnum < 9200 || obj->pIndexData->vnum > 9299)
              &&  (mob->pIndexData->vnum != 8002))
			fprintf(stderr,
...
void randomize_entrances( int code )
{
	...
	if ((portal != NULL) && (portal->item_type == ITEM_EXIT))
	{
	    ...
	    toroom = ROOM_VNUM_CHAIN_START;
	    chain = ROOM_VNUM_CHAIN_START;
	}
	pToRoomIndex = get_room_index( toroom );
	for ( ; ; )
	{
	    ...
	    {
		if (((pexit->u1.to_room->vnum >= 900)
		&&   (pexit->u1.to_room->vnum <= 999)
		&& (pexit->u1.to_room->vnum != 909)
		)
		//&& pexit->u1.to_room->vnum != 326)
		)
		{
