...
char * get_ascii (ROOM_INDEX_DATA * pRoom, bool fWall)
{
    ...
    if (!fWall)
    {
        switch (pRoom->sector_type)
        {
            ...
            case SECT_UNDERGROUND:   return "{y.";
            case SECT_DESERT:  return "{Y.";
            case SECT_UNUSED:   return "{M.";
...
void create_map (CHAR_DATA * ch)
{
    ...
    memset (g_list, (int)NULL, sizeof(g_list));
    memset (c_list,(int)NULL, sizeof(c_list));
...
void display_map (CHAR_DATA * ch)
{
    ...
    memset (map, (int)NULL,sizeof(map));
    create_map (ch);
