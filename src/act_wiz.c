	    ...
	    long flag, long flag_skip, int min_level) 
{
	...
	&&  d->character != ch)
	    ...
	    if (IS_SET(d->character->wiznet,WIZ_PREFIX))
	  	send_to_char("{W--{Y>{x ",d->character);
...
void do_ostat( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "ELevel: %d \n\r",
      obj->pIndexData->effective_level );
    send_to_char( buf, ch );
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf,
	"Lv: %d  Elvl: %d Class: %s  Align: %d  Exp: %ld\n\r",
	victim->level,       
	!IS_NPC(victim) ? "-1" : victim->pIndexData->effective_level,   
    ...
    {
	sprintf( buf,
	    "Thirst: %d  Hunger: %d  Full: %d  Drunk: %d  GQps: %d  Quest points: %d  Nextquest: %d\n\r",
	    victim->pcdata->condition[COND_THIRST],
	    victim->pcdata->condition[COND_HUNGER],
	    victim->pcdata->condition[COND_FULL],
	    victim->pcdata->condition[COND_DRUNK],
	    victim->qps,
      victim->questpoints, victim->nextquest );
	add_buf(output,buf);
    }
    ...
    {
    	sprintf( buf, 
	    "Age: %d  Played: %d  Times connected: %d  Last Level: %d  Timer: %d\n\r",
	    get_age(victim), 
	    (int) (victim->played + current_time - victim->logon) / 3600, 
      victim->pcdata->timesplayed,
...
void do_knight( CHAR_DATA *ch, char *argument )
{
    ...
    {
        victim->level += 1;
        printf_to_char(victim, "You raise a level!! You are now %d!", victim->level);
...
void do_mset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "    hunger drunk full gquest security\n\r",	ch );
	send_to_char( "    qps nextquest\n\r",	ch );
	return;
    }
	...
	{
	    send_to_char( "Mana range is 0 to 500,000 mana points.\n\r", ch );
	    return;
	}
	...
	if ( value < 0 || value > 500000 )
	{
	    send_to_char( "Move range is 0 to 500,000 move points.\n\r", ch );
	    return;
	}
	...
	if ( IS_NPC(victim) )
	{
	    send_to_char( "NPC's can't do quests.\n\r", ch );
	    return;
	}
...
void do_pack ( CHAR_DATA *ch, char *argument )
{
    ...
    obj = create_object( get_obj_index(OBJ_VNUM_SURVIVAL_Z), 0 );
    obj->level = 5;
    obj_to_obj( obj, pack );
