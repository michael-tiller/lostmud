...
int class_lookup (const char *name)
{
   ...
   if (!str_prefix( name, "paladin"))
	return class_lookup("strider");
...
int apply_ac( int level, OBJ_DATA *obj, int iWear, int type )
{
	...
	switch ( type )
	{
	case AC_PIERCE:	return ((level/6)+5);
	case AC_BASH:	return ((level/6)+5);
	case AC_SLASH:	return ((level/6)+5);
	case AC_EXOTIC:	return (level/6);
	}
	return 0;
    }
...
OBJ_DATA *get_obj_list( CHAR_DATA *ch, char *argument, OBJ_DATA *list )
{
	...
	if ( (obj->item_type == ITEM_EXIT) && is_name(arg, obj->name) )
	{
		...
		if ( obj->item_type == ITEM_PORTAL && obj->pIndexData->vnum == OBJ_VNUM_CHAIN)
			return NULL;
...
char *comm_bit_name(int comm_flags)
{
    ...
    if (comm_flags & COMM_NOCGOSSIP	) strcat(buf, " no_cgossip");
...
char *room_bit_name(int room_flags)
{
    ...
    if (room_flags & ROOM_NOMAP	) strcat(buf, " nomap");
...
void check_spirit( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    spirit->dam_type = attack_lookup("chill");
    SET_BIT ( spirit->form,
        FORM_INSTANT_DECAY|FORM_UNDEAD|FORM_INTANGIBLE );
    SET_BIT ( spirit->act, ACT_AGGRESSIVE );
    SET_BIT ( spirit->affected_by, AFF_PASS_DOOR );

    sprintf(buf,"the vengeful spirit of %s",victim->short_descr);
    spirit->short_descr = str_dup(buf);

    sprintf(buf,"vengeful spirit %s",victim->name);
    spirit->name = str_dup(buf);

    char_to_room( spirit, ch->in_room );

    act("{YYou cower in fear as $N{Y appears before you!{x",ch,NULL,spirit,TO_CHAR);
    act("{Y$N{Y suddenly appears and attacks $n{Y!{x",ch,NULL,spirit,TO_ROOM);
