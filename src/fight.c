	    ...
	    bool show) 
{
     ...
     */
    if ( dam > 15000 && dt >= TYPE_HIT && !IS_IMMORTAL(ch))
    {
	bug( "Damage: %d: more than 15000 points!", dam );
	dam = 15000;
	if (!IS_IMMORTAL(ch))
	{
	    OBJ_DATA *obj;
	...
	{
	    act( "{c$n{c $T{x", victim, 0, victim->die_descr, TO_ROOM );
	}
	else
	{
	    act( "{c$n{c is {CDEAD!!{x", victim, 0, 0, TO_ROOM );
	}
	...
	gain_exp( victim, (5 * (exp_per_level(victim,victim->pcdata->points)
        ...
        sprintf( log_buf, "%s was slain by %s at %s [room %d]",
...
bool is_safe(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	else
	{
	    ...
	    if ( class_table[ch->class].tier == class_table[victim->class].tier ) {
...
bool check_shield_block( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    act( "{iYou block $n{i's attack with your shield.{x",  ch, NULL, victim, TO_VICT    );
    act( "{h$N{h blocks your attack with a shield.{x", ch, NULL, victim, TO_CHAR    );
...
bool check_dodge( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    act( "{iYou dodge $n{i's attack.{x", ch, NULL, victim, TO_VICT    );
    act( "{h$N{h dodges your attack.{x", ch, NULL, victim, TO_CHAR    );
...
*/
	...
	sprintf( buf, "{fYou receive {W%d{f experience points.{x\n\r", xp );
	...
	for ( obj = ch->carrying; obj != NULL; obj = obj_next )
	{
	    ...
	    {
		act( "{iYou are {Yzapped{i by $p.{x", ch, obj, NULL, TO_CHAR );
		act( "$n is {Yzapped{x by $p.",   ch, obj, NULL, TO_ROOM );
		obj_from_char( obj );
		obj_to_room( obj, ch->in_room );
	    }
	}
...
int xp_compute( CHAR_DATA *gch, CHAR_DATA *victim, int total_levels )
{
    ...
    // make higher tiers level slower
    if (!IS_NPC(gch)){
	    if (class_table[gch->class].tier ==TIER_03) {
	      level_range -= 5;
	    } else if (class_table[gch->class].tier == TIER_02) {
	      level_range -= 3;
	    }  else {
	      level_range -= 0;
	    }
	...
	if (gch->level < 40)  /* make it a curve */
	    time_per_level = UMAX(time_per_level,(40 - gch->level));
/*
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim,int dam,int dt,bool immune )
{
    ...
    else if ( dam <=   40 ) { vs = "scratch";	vp = "scratches";	}
    else if ( dam <=   80 ) { vs = "graze";	vp = "grazes";		}
    else if ( dam <=  120 ) { vs = "hit";	vp = "hits";		}
    else if ( dam <=  160 ) { vs = "injure";	vp = "injures";		}
    else if ( dam <=  200 ) { vs = "wound";	vp = "wounds";		}
    else if ( dam <=  240 ) { vs = "maul";       vp = "mauls";		}
    else if ( dam <=  280 ) { vs = "decimate";	vp = "decimates";	}
    else if ( dam <=  300 ) { vs = "devastate";	vp = "devastates";	}
    else if ( dam <=  340 ) { vs = "maim";	vp = "maims";		}
    else if ( dam <=  420 ) { vs = "MUTILATE";	vp = "MUTILATES";	}
    else if ( dam <=  560 ) { vs = "DISEMBOWEL";	vp = "DISEMBOWELS";	}
    else if ( dam <=  610 ) { vs = "DISMEMBER";	vp = "DISMEMBERS";	}
    else if ( dam <=  700 ) { vs = "MASSACRE";	vp = "MASSACRES";	}
    else if ( dam <=  800 ) { vs = "MANGLE";	vp = "MANGLES";		}
    else if ( dam <=  900 ) { vs = "*** DEMOLISH ***";
			     vp = "*** DEMOLISHES ***";			}
    else if ( dam <= 1000 ) { vs = "*** DEVASTATE ***";
			     vp = "*** DEVASTATES ***";			}
    else if ( dam <= 2000)  { vs = "=== OBLITERATE ===";
			     vp = "=== OBLITERATES ===";		}
    else if ( dam <= 3250)  { vs = ">>> ANNIHILATE <<<";
			     vp = ">>> ANNIHILATES <<<";		}
    else if ( dam <= 4500)  { vs = "<<< ERADICATE >>>";
			     vp = "<<< ERADICATES >>>";			}
	...
	if (ch  == victim)
	{
	    sprintf( buf1, "{k$n {k%s{k $melf%c{x",vp,punct);
	    sprintf( buf2, "{hYou {h%s{h yourself%c{x",vs,punct);
	}
	else
	{	
		sprintf( buf1, "{k$n {k%s{k $N%c{x",  vp, punct );

		if (IS_SET(ch->act,PLR_AUTODAMAGE) && dam > 0) {
			sprintf( buf2, "{hYou {h%s{h $N%c {x[%d]{x", vs, punct, dam );
		} else {
			sprintf( buf2, "{hYou {h%s{h $N%c{x", vs, punct );	
		}
			
		if (IS_SET(victim->act,PLR_AUTODAMAGE) && dam > 0) {
			sprintf( buf3, "{i$n {i%s {iyou%c {x[%d]{x", vp, punct, dam );
		} else {
			sprintf( buf3, "{i$n {i%s {iyou%c{x", vp, punct );		
		}
	...
	if (immune)
	{
	    if (ch == victim)
	    {
		sprintf(buf1,"{k$n {kis unaffected by $s own %s.{x",attack);
		sprintf(buf2,"{hLuckily, you are immune to that.{x");
	    } 
	    else
	    {
	    	sprintf(buf1,"{k$N {kis unaffected by $n's %s!{x",attack);
	    	sprintf(buf2,"{h$N {his unaffected by your %s!{x",attack);
	    	sprintf(buf3,"{i$n{i's %s is powerless against you.{x",attack);
	    }
	}
	else
	{
	    if (ch == victim)
	    {
			sprintf( buf1, "{k$n{k's %s{k %s{k $m%c{x",attack,vp,punct);

			if (IS_SET(ch->act,PLR_AUTODAMAGE) && dam > 0) {
				sprintf( buf2, "{hYour %s{h %s{h you%c {x[%d]{x",attack,vp,punct, dam);
			} else {
				sprintf( buf2, "{hYour %s{h %s{h you%c{x",attack,vp,punct);
			}
	    }
	    else
	    {
			sprintf( buf1, "{k$n{k's %s{k %s{k $N%c{x",  attack, vp, punct );

			if (IS_SET(ch->act,PLR_AUTODAMAGE) && dam > 0) {
				sprintf( buf2, "{hYour %s{h %s{h $N%c {x[%d]{x",  attack, vp, punct, dam );
			} else {
	    		sprintf( buf2, "{hYour %s{h %s{h $N%c{x",  attack, vp, punct );
			}			
			
			if (IS_SET(victim->act,PLR_AUTODAMAGE) && dam > 0) {
				sprintf( buf3, "{i$n{i's %s{i %s{i you%c {x[%d]{x", attack, vp, punct, dam );
			} else {
	    		sprintf( buf3, "{i$n{i's %s{i %s{i you%c{x", attack, vp, punct );
			}
		}
	}
	}
	...
	act( "$n has {Yfled{x!", ch, NULL, NULL, TO_ROOM );
        if ( !IS_NPC(ch) )
        {
            ...
            {    
                ...
                {
                    send_to_char( "The {RWrath of Chaos {YZAPS{x your butt on the way out!\n\r", ch);
...
void do_slay( CHAR_DATA *ch, char *argument )
{
	...
	act("{h$n{h is DEAD!!!{x",ch,NULL,victim,TO_VICT);
	act("{k$n{h is DEAD!!!{x", ch,NULL,victim,TO_NOTVICT);
