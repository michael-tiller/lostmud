...
//	signal(SIGTERM, (void *) bailout);
    ...
    boot_db();
    sprintf( log_buf, "Lost Legends is ready to rock on port %d.", port );
    log_string( log_buf );
...
void close_socket( DESCRIPTOR_DATA *dclose )
{
    ...
    {
	send_to_desc( "Your victim has left the game.\n\r", dclose->snoop_by );
    }
...
bool process_output( DESCRIPTOR_DATA *d, bool fPrompt )
{
    ...
    if ( !merc_down )
		if ( d->showstr_point )
			send_to_desc( "[Hit Any Key to continue]\n\r", d );
	...
	ch = d->character;
	    ...
	    char buf[MAX_STRING_LENGTH];
			...
			if (!IS_SET(ch->act,PLR_AUTODAMAGE)) {
				...
				if (percent >= 100)
					sprintf(wound,"is in excellent condition.{x");
				else if (percent >= 80)
					sprintf(wound,"has a few scratches.{x");
				else if (percent >= 60)
					sprintf(wound,"has some small wounds and bruises.{x");
				else if (percent >= 40)
					sprintf(wound,"has quite a few wounds.{x");
				else if (percent >= 20)
					sprintf(wound,"has some big nasty wounds and scratches.{x");
				else if (percent >= 10)
					sprintf(wound,"looks pretty hurt.{x");
				else if (percent >= 0)
					sprintf(wound,"is in awful condition.{x");
				else
					sprintf(wound,"is bleeding to death...");
			} else {

				if (percent >= 100)
					sprintf(wound,"is in excellent condition. {x[%d%%]{x", percent);
				else if (percent >= 80)
					sprintf(wound,"has a few scratches. {x[%d%%]{x", percent);
				else if (percent >= 60)
					sprintf(wound,"has some small wounds and bruises. {x[%d%%]{x", percent);
				else if (percent >= 40)
					sprintf(wound,"has quite a few wounds. {x[%d%%]{x", percent);
				else if (percent >= 20)
					sprintf(wound,"has some big nasty wounds and scratches. {x[%d%%]{x", percent);
				else if (percent >= 10)
					sprintf(wound,"looks pretty hurt. {x[%d%%]{x", percent);
				else if (percent >= 0)
					sprintf(wound,"is in awful condition. {x[%d%%]{x", percent);
				else
					sprintf(wound,"is bleeding to death...");

			}
 
            sprintf(buf,"{f%s{f %s{x\n\r", 
	            IS_NPC(victim) ? victim->short_descr : victim->name,wound);
	    buf[0] = UPPER(buf[0]);
	    send_to_char(buf, ch);
	    if (victim->stunned)
	    {
		sprintf(buf,"{f%s{f is stunned.{x\n\r", 
...
/*            send_to_desc( buf, d); */
    ...
    if (IS_SET (ch->act, PLR_AUTODIG))
        send_to_char ("{R[AUTODIG ENABLED]{x\n\r", ch);
...
//    char strsave[MAX_INPUT_LENGTH];
		...
		if ( argument[0] == '\0' || UPPER(argument[0]) == 'Y' ) {
			...
			
			{
				...
				switch (number_range(0,4))
				{
					default:
					if ( help_greetinga[0] == '.' )
						send_to_desc( help_greetinga+1, d );
					else
						send_to_desc( help_greetinga  , d );
					break;
					case 0:
					if ( help_greetinga[0] == '.' )
						send_to_desc( help_greetinga+1, d );
					...
					case 1:
					if ( help_greetingb[0] == '.' )
						send_to_desc( help_greetingb+1, d );
					else
						send_to_desc( help_greetingb  , d );
					break;
					case 2:
					if ( help_greetingc[0] == '.' )
						send_to_desc( help_greetingc+1, d );
					else
						send_to_desc( help_greetingc  , d );
					break;
					case 3:
					if ( help_greetingd[0] == '.' )
						send_to_desc( help_greetingd+1, d );
					else
						send_to_desc( help_greetingd  , d );
					break;
					case 4:
					if ( help_greetinge[0] == '.' )
						send_to_desc( help_greetinge+1, d );
					else
						send_to_desc( help_greetinge  , d );
					break;
				}
			}
	...
	if ( !check_parse_name( argument ) )
	{
	    ...
	    send_to_desc( "(If you've used this name here before, and are no\n\r", d);
	    send_to_desc( " longer able to, it may be because we've added a\n\r", d );
	    send_to_desc( " new mobile that uses the same name. Log in with\n\r", d );
	    send_to_desc( " a new name, and let an IMM know, and we will fix it.)\n\r", d );
	    send_to_desc( "\n\rName: ", d );
	    return;
	}
...
/* RT code for breaking link */
	...
	/* another instance where simplicity is
     * a blessing     -- Seronis  ********/
	if ( !IS_SET(ch->pcdata->ctier, class_table[iClass].tier) )
	{
		write_to_buffer( d,"That class is not allowed to your tier.\n\rWhat IS your class? ", 0 );
		return;
	}
        ch->class = iClass;
...
case CON_DEFAULT_CHOICE:
	...
	write_to_buffer( d, 
    "\n\rWelcome to Lost Legends.  Please do not feed the immortals.\n\r",
	    0 );
	ch->next	= char_list;
	char_list	= ch;
...
ch->pcdata->timesplayed ++;
	...
	ch->pcdata->ctier = class_table[ch->class].tier;
...
bool check_parse_name( char *name )
{
     ...
     * Reserved words.
     */
    if ( is_name( name, "all auto immortal immortals self"
						"someone something the you none"
						"loner unlinked") )
	return FALSE;
...
//char *colour( char type, DESCRIPTOR_DATA *d )
{
	/*
	CHAR_DATA *ch;
	...
	case 'D':
        ...
        case '*':
                ...
                { default : sprintf(clcode, C_D_GREY); break;
                  case 1 : sprintf(clcode, C_RED); break;
                  case 2 : sprintf(clcode, C_B_RED); break;
                  case 3 : sprintf(clcode, C_GREEN); break;
                  case 4 : sprintf(clcode, C_B_GREEN); break;
                  case 5 : sprintf(clcode, C_YELLOW); break;
                  case 6 : sprintf(clcode, C_B_YELLOW); break;
                  case 7 : sprintf(clcode, C_BLUE); break;
                  case 8 : sprintf(clcode, C_B_BLUE); break;
                  case 9 : sprintf(clcode, C_MAGENTA); break;
                  case 10 : sprintf(clcode, C_B_MAGENTA); break;
                  case 11 : sprintf(clcode, C_CYAN); break;
                  case 12 : sprintf(clcode, C_B_CYAN); break;
                  case 13 : sprintf(clcode, C_WHITE); break;
                  case 14 : sprintf(clcode, C_B_WHITE); break;
                  case 15 : sprintf(clcode, C_D_GREY); break; }
            break;
	...
	case 'o':	/* ooc */
            if (ch->color_ooc)
            {
                sprintf( clcode, colour_channel(ch->color_ooc, ch));
            }
            else
            {
                sprintf( clcode, C_B_WHITE);
