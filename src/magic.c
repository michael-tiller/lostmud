...
void spell_cause_light( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    num = level / 20;
    type = 8;
    if (level > 100) type = 10;
    bonus = level;

    damage( ch, victim, dice(num, type) + bonus, sn,DAM_HARM,TRUE);
    return;
}
...
void spell_create_food( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    act( "$p falls from the sky.", ch, food, NULL, TO_ROOM );
    act( "$p falls from the sky.", ch, food, NULL, TO_CHAR );
    return;
}
...
void spell_cure_critical( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    type = 12;
    if (level > 100) type = 20;
    bonus = 200 + (level * 1.5f);
    heal = dice(num, type) + bonus;
    ...
    if (victim->max_hit == victim->hit)
      ...
      printf_to_char(victim, "You feel much better! [%d]\n\r", heal );
    if ( ch != victim )
	    send_to_char( "Ok.\n\r", ch );
    return;
}
...
void spell_cure_light( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    if (victim->max_hit == victim->hit)
      ...
      printf_to_char(victim, "You feel a little better! [%d]\n\r", heal );
    if ( ch != victim )
	    send_to_char( "Ok.\n\r", ch );
    return;
}
...
void spell_cure_serious( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    if (victim->max_hit == victim->hit)
      ...
      printf_to_char(victim, "You feel better! [%d]\n\r", heal );
    if ( ch != victim )
	    send_to_char( "Ok.\n\r", ch );
    return;
}
...
void spell_dispel_evil( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    if ( IS_GOOD(victim) )
    {
	act( "Gaia protects $N.", ch, NULL, victim, TO_ROOM );
	return;
    }
...
void spell_dispel_good( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    if ( IS_EVIL(victim) )
    {
        act( "Chaos protects $N.", ch, NULL, victim, TO_ROOM );
        return;
    }
...
void spell_earthquake( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    send_to_char( "The earth rumbles beneath your feet!\n\r", ch );
    act( "$n makes the earth rumble and shake.", ch, NULL, NULL, TO_ROOM );
    ...
    bonus = level;
			...
			damage( ch,vch,bonus + dice(num, type), sn, DAM_BASH,TRUE);
    }
	    }
	    continue;
	}
	...
	if ( vch->in_room->area == ch->in_room->area )
	    send_to_char( "The earth rumbles and shakes.\n\r", vch );
    }
...
void spell_heal( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    type = 10;
    if (level > 100) type = 20;
    bonus = 500 + (level * 1.75f);
    heal = dice(num, type) + bonus;

    victim->hit = UMIN( victim->hit + heal, victim->max_hit );
    update_pos( victim );
    printf_to_char(victim, "A warm feeling fills your body. [%d]\n\r", heal );
    if ( ch != victim )
	send_to_char( "Ok.\n\r", ch );
    return;
}
...
void spell_refresh( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    type = 4;
    if (level > 100) type = 8;
    bonus = 100 + (level * 1.5f);
    refresh = dice(num, type) + bonus;

    victim->move = UMIN( victim->move + refresh, victim->max_move );
